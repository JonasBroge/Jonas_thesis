...
P_scen =  Array{Float64}(undef, maximum(gen_df.gen), maximum(gen_df.time),maximum(gen_df.scenario))
C_scen =  Array{Float64}(undef, maximum(gen_df.gen), maximum(gen_df.time),maximum(gen_df.scenario))
for i in 1:nrow(gen_df)
    P_scen[gen_df.gen[i],gen_df.time[i],gen_df.scenario[i]] = gen_df.max[i]
    C_scen[gen_df.gen[i],gen_df.time[i],gen_df.scenario[i]] = gen_df.cost[i]
end
...
#Expected value of P and C
P = mean(P_scen,dims=3)
C = mean(C_scen,dims=3)
...
@constraint(FAP1, FAP_Stor4, e[H] == e_H_full)                   # (xi)  
...
## -------------------- Time Horizon 2 -----------------
### WHEN TIME HORIZON 2 IS REACHED AND THE UNCERTAINTY IS REALIZED, PRODUCTION 
# Sets
T2 = (H+1):time
L = 1:1
G = 1:2
...
@constraint(FAP2, FAP_Gen[g in G, t in T2], 0 <= p[g,t] <= P_scen[g,t,2])                                     # 2c (μ_low[g,t], μ_up[g,t])
@constraint(FAP2, FAP_Load[l in L, t in T2], 0 <= d[l,t] <= D[l,t])                                    # 2d (X_low[g,t], X_up[g,t])
## Storage Constraints (1a-1d)
@constraint(FAP2, FAP_Stor1[t in T2],   0 <= e[t] <= S)             #1a (v_low[t] , v_up[t])
@constraint(FAP2, FAP_Stor2[t in T2;t!=H+1],   e[t] == e[t-1]+b[t])   #1b (rho[t])
@constraint(FAP2, FAP_Stor3, e[H+1] == e[H]+b[H+1])                  #1c (rho[1]) 
@constraint(FAP2, FAP_Stor4, e[H] == e_H_full)
