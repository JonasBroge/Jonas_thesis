...
# This function can be used after the base-case scenario has been defined, and its variables initialized.
### The function runs with the basecase model inputs for the full-horizon, and FAP_t1, 
### and then changes the max output of a desired generator for FAP_t2.
using JuMP
using HiGHS
using CSV
using DataFrames
using Statistics
function production_change(gen=1,time=2,max=2)
    H=1
    T = 1:size(D)[2]
    L = 1:size(D)[1]
    G = 1:size(P)[1]
    ...
    if termination_status(full) == MOI.OPTIMAL
        ...
        e_H_full = value(e[H])
        println("The value of ρ[H+1]*=" , dual(Balance[t]))
        println("Storage level at end of t=", t, " is e=", value(e[t]))
    else
    ...
    @constraint(FAP2, FAP_Gen[g in G, t in T2], 0 <= p[g,t] <= P_scen[g,t])                                     # 2c (μ_low[g,t], μ_up[g,t])
