    ...
    @objective(full, Max, sum( sum(U[l,t]*d[l,t,s] for l in L) -  sum(C[g,t]*p[g,t,s] for g in G) for t in T) )
    ### Subject to
    ## Market Constraints (2b-2d)
    @constraint(full, Balance[t in T], sum(d[l,t,s] for l in L) + b[t,s] - sum(p[g,t,s] for g in G) == 0 )  # 2a
    @constraint(full, Gen[g in G, t in T], 0 <= p[g,t] <= P[g,t])
    @constraint(full, Load[l in L, t in T], 0 <= d[l,t] <= D[l,t]) 
    ## Storage Constraints (1a-1d)
    @constraint(full, Stor1[t in T, s in S],   0 <= e[t, s] <= S)          #1a (v_low[t] , v_up[t])
    @constraint(full, Stor2[s in S, t in T;t!=1],   e[t, s] == e[t-1, s]+b[t, s])     #1b (rho[t])
    @constraint(full, Stor3, e[1, s] == E_init+b[1, s])                 #1c (rho[1])
    ...
    @constraint(FAP2, FAP_Load[l in L, t in T], 0 <= d[l,t] <= D[l,t])                                     # 2d (X_low[g,t], X_up[g,t])
    ## Storage Constraints (1a-1d)
